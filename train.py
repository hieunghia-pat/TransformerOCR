...
def train():
    ...
    for stage in range(len(folds)):
        ...
        for epoch in range(config.max_epoch):
            ...
            scores = run_epoch([folds[-1]], False, "Evaluation", epoch, model, 
                SimpleLossCompute(model.generator, criterion, None), metric, tracker)

            if best_score["cer"] < scores["cer"]:
                best_score["cer"] = scores["cer"]
                best_score["wer"] = scores["wer"]
                torch.save(os.path.join(config.checkpoint_path, f"best_model_stage_{stage+1}.pth"))

            torch.save(os.path.join(config.checkpoint_path, f"last_model_stage_{stage+1}.pth"))
